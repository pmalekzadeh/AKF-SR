...
def count_coop_agent_collisions(env):
    """
    Returns a count of all collisions between two cooperating agents.
    """
    counts_ = []
    for i in range(env.n):
        collide_i = 0
        for j in range(env.n):
            is_collide = is_collision(env.agents[i], env.agents[j])
            if is_collide and env.agents[i].adversary is env.agents[j].adversary:
                collide_i += 1
        counts_.append(collide_i)
    return counts_
